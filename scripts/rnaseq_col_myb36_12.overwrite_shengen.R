...
df_high <- gofilter(x = ego,level = 4) 
p_ego <- emapplot(df_high,color = "p.adjust",showCategory = 100) + 
  scale_color_paletteer_c(package = "viridis",palette = "plasma",na.value = "#BFBFBF") 

oh.save.pdf(p = p_ego,outname = "emap_overwrite.pdf",
            outdir = "../figures/",width = 8,height = 6)

## Save the boxplot
oh.save.pdf(p = p_boxplot,outname = "boxplot_overwrite.pdf",outdir = "../figures/",width = 4,height = 6)
rm(list=ls())
dev.off()
