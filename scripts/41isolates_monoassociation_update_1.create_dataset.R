...
df_mroot$Strains  <- df_mroot$Strains %>% 
  ...
  gsub(pattern = "^RMF32$",replacement = "RMF329") %>%
  gsub(pattern = "^RMF32HK$",replacement = "RMF329HK") %>%
  gsub(pattern = "-",replacement = "")
...
############################
saveRDS(object = Raw,
        file = "../cleandata/dat_41isolatesmonoassociation_cfu_suberin_primroot_dryweight_ionome.updated.RDS")
#Clean workspace
dev.off()
gc()
rm(list=ls())
