...
Dat_mono <- readRDS(file = "../cleandata/dat_41isolatesmonoassociation_cfu_suberin_primroot_dryweight_ionome.updated.RDS")
...
df_sub$Norm_Distance_tip_cz <- df_sub$Norm_Distance_tip_cz %>% scale
...
for(st in df_sub$Strains %>% unique){
  ...
  m <- car::leveneTest(as.numeric(Norm_Distance_tip_cz) ~ Strains,data = df_temp) 
  pval <- m$`Pr(>F)`[1]
  if(pval < 0.05){
...
df_sub$Norm_Main_root_elongation <- df_sub$Norm_Main_root_elongation %>% scale
df_nb <- df_sub %>% subset(Strains == "NB") %>% droplevels
df_sub <- df_sub %>% subset(Strains != "NB") %>% droplevels
...
for(st in df_sub$Strains %>% unique){
  ...
  df_temp <- rbind(df_temp_nb,df_temp) 
  df_temp$Strains <- df_temp$Strains   %>% factor
  m <- car::leveneTest(as.numeric(Norm_Main_root_elongation) ~ Strains,data = df_temp) 
  pval <- m$`Pr(>F)`[1]
  if(pval < 0.05){
    m1 <- t.test(x = x,y = y,exact = F,var.equal = F)
...
df_sub$Norm_WeightbyPlant <- df_sub$Norm_WeightbyPlant %>% scale
df_nb <- df_sub %>% subset(Strains == "NB") %>% droplevels
df_sub <- df_sub %>% subset(Strains != "NB") %>% droplevels
...
for(st in df_sub$Strains %>% unique){
  ...
  df_temp <- rbind(df_temp_nb,df_temp) 
  df_temp$Strains <- df_temp$Strains   %>% factor
  m <- car::leveneTest(as.numeric(Norm_WeightbyPlant) ~ Strains,data = df_temp) 
  pval <- m$`Pr(>F)`[1]
  if(pval < 0.05){
    m1 <- t.test(x = x,y = y,exact = F,var.equal = F)
...
write.table(x = allRes,file = "../cleandata/res_41_suberinraw_primroot_weight_vsnb.zscore.updated.tsv",
