...
Dat <- readRDS(file = "../cleandata/deseq2_models_rootbarriers_amplicon_bacteria_syncom_stresses.RDS")

###### Root #########
dds <- Dat$RootStress

#Everything against Full
stresses <- dds$Stress %>% as.character %>% unique %>% 
  grep(pattern = "Full",invert = T,value = T)

Res <- NULL
for(stress in stresses){
  cat("Working on ",stress,"\n")
  cont <- c("Stress",stress,"Full")
...
Res$Stress <- Res$Contrast %>% gsub(pattern = "_.*",replacement = "")
...
Res$Stress <- Res$Stress %>% 
  factor(levels = c("-P","-Mn","-Mo","-Zn","-K","+NaCl","-Fe"))
...
df_sub <- with(df_sub,order(NormProp)) %>%
  df_sub[.,]
...
#Now do the plotting
p <- ggplot(data = Res,aes(Stress,Id)) +
...
#Inverted version
p <- ggplot(data = Res,aes(Id,Stress)) +
  geom_raster(aes(fill =log2FoldChange ), stat = "identity") +
  geom_tile(aes(color = Significance),fill = '#00000000', size = 1,width = 0.85,height = 0.85) + 
  scale_color_manual(values = c("#00000000","black"))+
...
oh.save.pdf(p = p,outname = "heatmap_stresses_syncom_by_fraction.pdf",
            outdir = "../figures/",width = 14,height =7)
...
oh.save.pdf(p = p,outname = "bubuplot_log2foldchange_stresses_syncom_by_fraction.pdf",
...
acast(data = Res,formula =Group~Fraction + Stress,fun.aggregate = sum,value.var = "CountUp" )  %>%
  rowSums()
acast(data = Res,formula =Group~Fraction + Stress,fun.aggregate = sum,value.var = "CountDown" )  %>%
  rowSums()

melt_up <- dcast(data = Res,formula =Group~Fraction + Stress,fun.aggregate = sum,value.var = "CountUp" )  %>%
melt

melt_down <- dcast(data = Res,formula =Group~Fraction + Stress,fun.aggregate = sum,value.var = "CountDown" )  %>%
   melt
...
melted$Stress <- melted$variable %>% gsub(pattern = ".*_",replacement = "")
...
oh.save.pdf(p = p,outname = "barplot_amplicon_stresses_groups.pdf",
